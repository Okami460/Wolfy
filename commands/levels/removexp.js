...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!addxp) return message.reply(`Combien d'xp veut tu te give ?`) 
            ...
            Levels.subtractXp(target.id, message.guild.id, addxp)
            message.channel.send(`Suppression de **${addxp}** xp a ${target.user.username}`)
        }
    }
}